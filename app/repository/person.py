...
class PersonRepository(BaseRepository):
    def basic_table_config(self):
        self.cursor.execute(
            "CREATE TABLE IF NOT EXISTS Person (id TEXT, name TEXT, last_name TEXT, email TEXT)"
        )

    def insert(self, person: Person):
        insert_stmt = "INSERT INTO Person (id, name, last_name, email) VALUES (?,?,?,?);"
        data_tuple = (str(person.id), person.name, person.last_name, person.email)
